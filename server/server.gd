...
func _start_match():
	...
	var human_ids = queue.slice(0, MAX_PLAYERS)
	for id in human_ids:
		queue.erase(id)
	...
	#match_scene.set_script(preload("res://server/manager.gd"))
	#match_scene.get_child(0).queue_free()
	
	# Isolate networking for this room
	var room_api = SceneMultiplayer.new()
	room_api.multiplayer_peer = multiplayer.multiplayer_peer
	
	# Matches container must exist in Main.tscn
	...
	for id in human_ids:
		print('inviting ', id)
		Network.setup_client_room.rpc_id(id, room_name, human_ids)
		#rpc_id(id, "setup_client_room", room_name, human_ids)
	
	match_scene.initialize_game(human_ids)
